 ...
 *
 * Daftar kunci yang sah - dan tabel warna aksen - DITURUNKAN dari VIEWS, TABS,
 * dan ACCENTS, tidak ditulis ulang di sini. Versi sebelumnya menyalin daftarnya sebagai teks, lalu tertinggal saat
...
const BOOT = `(function(){try{
  ...
  var M=${JSON.stringify(ACCENTS)};
  if(a&&M[a]){d.style.setProperty('--accent',M[a][0]);
    d.style.setProperty('--accent-rgb',M[a][1]);
    d.style.setProperty('--accent-ink-l',M[a][2]);
    d.style.setProperty('--accent-ink-d',M[a][3]);d.dataset.accent=a;}
}catch(e){}})();`;
